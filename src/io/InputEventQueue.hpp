...
public:
	...
	{ 
		assert(!queue_.empty() && "[에러] InputEvent큐가 비어있어 front를 확인할 수 없습니다.");
		return queue_.front();
	}

	InputEvent pop() 
	{
		assert(!queue_.empty() && "[에러] InputEvent큐가 비어있어 pop할 수 없습니다.");
...
private:
	...
	InputEventQueue(const InputEventQueue&) = delete; //복사 금지
	InputEventQueue& operator=(const InputEventQueue&) = delete; //대입 금지
};
